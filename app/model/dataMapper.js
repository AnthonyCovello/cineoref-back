...
// ------------- USER ----------
    ...
    async getUsers() {
      const query = {
        text : `SELECT public.user.id, username, email, birthday, to_char(created_at, 'dd/mm/yyyy') AS created_date, role.name AS role, grade.name AS grade FROM "user"
        JOIN "grade"
  ...
  async editArtist(body){
   ...
   async createShow(show) {
    ...
    async getRequest(){
      const query = {
        text : `SELECT reference.id, reference.ref, show.name AS show, show.category AS media, public.character.name AS character, artist.name AS artist, public.user.username AS user
        FROM public.reference
        JOIN public.show
        on reference.show_id = show.id
        JOIN public.artist
